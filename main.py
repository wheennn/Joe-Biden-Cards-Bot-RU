...
issued_id = int(os.getenv("ISSUED_USERS_ID"))
...
async def season_rewards_giveaway(recipient_id: int, quantity: int):
 cursor.execute("SELECT full_name, xp, pack_stage FROM users WHERE user_id = ?", (recipient_id,))
 row = cursor.fetchone()
 name, xp, stg = row if row else ("", 0, 0)
 cursor.execute("SELECT COUNT(*) FROM users WHERE xp > ? AND user_id NOT IN (?, ?)", (xp, dev_id, dev_mini_id))
 row = cursor.fetchone()
 placement = row[0] + 1 if row else 0
 if stg < 2:
  currency = "fpacks"
  pack_name = "обычный набор карт" if quantity == 1 else "обычных набора карт"if quantity in (2, 3, 4) else "обычных наборов карт" if quantity == 5 else "обычных наборов карт"
 elif stg >= 2:
  currency = "spacks"
  pack_name = "расширенный набор карт" if quantity == 1 else "расширенных набора карт" if quantity in (2, 3, 4) else "расширенных наборов карт" if quantity == 5 else "расширенных наборов карт"
 cursor.execute(f"UPDATE users SET {currency} = {currency} + ? WHERE user_id = ?", (quantity, recipient_id))
 db.commit()
 text = f"<b>{name}</b>, вы получили <b>x{quantity} {pack_name}</b> за достижение <b>топ-{placement} мира</b> в прошлом сезоне!"
 try:
  await bot.send_message(chat_id=recipient_id, text=text)
  report = f"✅ <b>{name}</b> успешно <b>получил</b> сообщение о выдаче награды за топ-{placement} мира\n"
 except Exception as e:
  report = f"❌ <b>{name}</b> <b>не получил</b> сообщение о выдаче награды за топ-{placement} мира: {e}\n"
 return report

async def compensation_for_issued_player():
 cursor.execute("SELECT fpack_unlocked_cards, full_name FROM users WHERE user_id = ?", (issued_id,))
 row = cursor.fetchone()
 funlocked, name = row if row else (0, None)
 if funlocked < fpack_full_coll:
  cursor.execute("UPDATE users SET pack_stage = 1, xp = xp + 50, fpacks = fpacks + 5 WHERE user_id = ?", (issued_id,))
 db.commit()
 text = (
  f"{escape(name)}, вы получили <b>компенсацию</b> в виде 50 XP и 5 обычных наборов карт за критический баг с неверным значением стадии прохождения, произошедший на вашей учётной записи\n\n"
  "Данный баг был <b>успешно исправлен</b> и больше не должен вызывать дискомфорт.\n\n"
  "Желаем вам <b>честной</b> и <b>приятной</b> игры, а также извиняемся за случившиеся неудобства!"
 )
 try:
  await bot.send_message(chat_id=issued_id, text=text)
  await bot.send_message(chat_id=dev_id, text=f"✅ <b>{name}</b> успешно получил сообщение с компенсацией")
 except Exception as e:
  print(f"Неудача при попытке отправки сообщения с компенсацией пользователю {issued_id}: {e}")
...
async def season_dispatcher(): # ДИСПЕТЧЕР СЕЗОНОВ v0.1.2+
 ...
 while True:
  ...
  cursor.execute("SELECT current_season FROM season_info") # ЧАСТЬ С ПРЕДОСТАВЛЕНИЕМ ДАННЫХ О СЕЗОНАХ
  row = cursor.fetchone()
  season = row[0] if row else 0
  if 1777203780 <= int(time.time()) <= 1780912800 and season == 0: # сезон 0
  ...
  if (season_end - int(time.time())) in range(86398, 86402): # ЧАСТЬ С РАССЫЛКОЙ О ПОЯВЛЕНИИ МАРКЕТА
   cursor.execute("SELECT user_id, full_name FROM users")
   users = cursor.fetchall()
   for user in users:
    try:
     text = "🛍 Остался 1 день до окончания сезона, поэтому временный маркет возвращается!\n\nВ нём доступны акции на <b>Обычные</b> и <b>Расширенные Наборы Карт</b> со скидками до <b>-10%</b>.\n\nЗакупайтесь или изучайте полный список товаров с помощью команды /market"
     await bot.send_message(chat_id=user[0], text=text)
     await asyncio.sleep(0.1)
    except Exception as e:
     print(f"Не удалось отправить уведомление для {user[1]}: {e}")
   await asyncio.sleep(6)

  if int(time.time()) in range(season_end - 2, season_end + 2): # ЧАСТЬ С ВЫДАЧЕЙ НАГРАД ПОСЛЕ ОКОНЧАНИЯ СЕЗОНА
   dev_report = f"<b>ОТЧЁТ О ПОЛУЧЕНИИ СООБЩЕНИЙ И НАГРАД ПОСЛЕ ОКОНЧАНИЯ {season}-ГО СЕЗОНА</b>\n\n<blockquote>"
   # Топ-1 мира: наборы
   ...
   if user:
    packq = 5 if season in (0, 1, 2) else 10
    cursor.execute("SELECT last_card_time, unlocked_cards FROM users WHERE user_id = ?", (user[1],))
    row = cursor.fetchone()
    prev_time, count_before = row if row else (0, 0)
    report = await season_rewards_giveaway(user[1], packq)
    dev_report += report
    # Топ-1 мира: пожизненный водолаз
    if season in (0, 1, 2):
     add_card(user[1], 6, 0, user[0])
     ...
     if row:
      ...
      db.commit()
      cursor.execute("UPDATE season_info SET winner = ? WHERE season = ?", (user[0], season))
      db.commit()
      if count_after == full_coll + 1 and count_before == full_coll:
       text = (
        f"<b>{escape(user[0])}</b>, вы собрали <b>абсолютно полную коллекцию Джо Байденов</b> из <b>{full_coll + 1} карт</b>! Теперь вы официально прошли <b>Joe Biden Cards и ваш общий достигнутый <b>прогресс равен 100%</b>!\n\n"
        "Поскольку бот активно обновляется и выходят новые карты, с целью баланса мы не можем выдать вам какую-либо награду, но знайте, <b>ваше имя</b> уже записано в <b>истории проекта!</b>\n\n"
        "Продолжайте играть, дожидаться новых обновлений и соревноваться за топ-1 мира!"
       )
       await bot.send_message(chat_id=user[1], text=text)
   # Топ-2 мира
   cursor.execute("SELECT user_id FROM users WHERE user_id NOT IN (?, ?) ORDER BY xp DESC LIMIT 1 OFFSET 1", (dev_id, dev_mini_id)) # ВЫДАЧА НАГРАДЫ ТОП 2 МИРА
   row = cursor.fetchone()
   id = row[0] if row else 0
   if id:
    report = await season_rewards_giveaway(id, 5)
    dev_report += report
   # Топ-3 мира
   cursor.execute("SELECT user_id FROM users WHERE user_id NOT IN (?, ?) ORDER BY xp DESC LIMIT 1 OFFSET 2", (dev_id, dev_mini_id)) # ВЫДАЧА НАГРАДЫ ТОП 3 МИРА
   row = cursor.fetchone()
   id = row[0] if row else 0
   if id:
    report = await season_rewards_giveaway(id, 3)
    dev_report += report
   # Топ 4-30
   cursor.execute("SELECT full_name, user_id, xp FROM users ORDER BY xp DESC LIMIT 30 OFFSET 3")
   members = cursor.fetchall()
   if members:
    for member in members:
     if member[2] > 0:
       report = await season_rewards_giveaway(member[1], 1)
       dev_report += report
       await asyncio.sleep(0.1)
    ...
    db.commit()
    await asyncio.sleep(6)
   dev_report += "</blockquote>"
   await bot.send_message(chat_id=dev_id, text=dev_report)
...
@dp.message(Command("all_info"))  # АДМИН КОМАНДА v0.1.1.3+ ! ЧАСТИЧНО НЕ РАБОТАЕТ v0.1.4.2+ !
async def showing_db_info(message: types.Message):
 ...
 for id in ids:
  ...
  stg_progress = fpack_progress if pack_stage < 2 else spack_progress
  ...
  how_much_is_needed = full_coll if unlocked_cards <= full_coll and get_user_card_count(id, 6) == 0 else full_coll + 1
 ...
 if len(text) >= 4096:
  parts = len(text) / 4096
  parts = int(parts) + 1 if parts > int(parts) else parts
  first_part, second_part, third_part, fourth_part, fifth_part, sixth_part, seventh_part, eighth_part, ninth_part, tenth_part = "", "", "", "", "", "", "", "", "", ""
  parts_massive = [first_part, second_part, third_part, fourth_part, fifth_part, sixth_part, seventh_part, eighth_part, ninth_part, tenth_part]
  index = 0
  for i in range(parts):
   first_coeff = index
   second_coeff = index + 1
   parts_massive[index] = text[first_coeff*4096:second_coeff*4096]
   index += 1
  html_tegs = ["<b>", "</b>", "<blockquote>", "</blockquote>"]
  html_tegs_lenght = [3, 4, 12, 13]
  for index_x in range(len(parts_massive)):
   x = parts_massive[index_x]
   if x == "":
    continue
   for index_y in range(len(html_tegs)):
    y = html_tegs[index_y]
    if index_y % 2 != 0:
     continue
    if index_x + 1 > 9 or index_y + 1 >= len(html_tegs):
     continue 
    tail = x[-html_tegs_lenght[index_y]:]
    broken_tag_start = tail.find("<")
    if broken_tag_start != -1 and tail[broken_tag_start:] != html_tegs[index_y]:
     actual_broken_index = len(x) - html_tegs_lenght[index_y] + broken_tag_start
     how_much_is_broken = html_tegs_lenght[index_y] - len(x[actual_broken_index:])
     x = x[:actual_broken_index]
     if index_x + 1 < len(parts_massive):
      parts_massive[index_x + 1] = parts_massive[index_x + 1][how_much_is_broken:]
     parts_massive[index_x] = x
    if x.count(y) > x.count(html_tegs[index_y + 1]):
     if index_x + 1 < len(parts_massive) and not parts_massive[index_x + 1].startswith(html_tegs[index_y]):
      x = x + html_tegs[index_y + 1]
      if index_x + 1 < len(parts_massive):
       if parts_massive[index_x + 1].startswith("<b>") or parts_massive[index_x + 1].startswith("<blockquote>"):
        first_space = parts_massive[index_x + 1].find("<", 1)
        if first_space != -1:
         parts_massive[index_x + 1] = parts_massive[index_x + 1][:first_space] + html_tegs[index_y] + parts_massive[index_x + 1][first_space:]
        else:
         parts_massive[index_x + 1] = parts_massive[index_x + 1] + html_tegs[index_y]
       else:
        parts_massive[index_x + 1] = html_tegs[index_y] + parts_massive[index_x + 1]
      parts_massive[index_x] = x
  index = 0
  for x in range(parts):
   if parts_massive[index]:
    await message.answer(parts_massive[index])
    await asyncio.sleep(0.1)
   index += 1
 else:
  await message.answer(text)
...
async def broadcast(message: types.Message):
 ...
 if int(time.time()) < 1782464400:
  ...
  await message.answer("Слишком поздно для рассылки.")
...
async def showing_market(message: types.Message):
 ...
 if 86400 >= left_before_s_end > 0:
  ...
  text = (
   ...
   "🃏 <b>x10 Обычных Наборов Карт</b> — <s>150</s> 135 XP (-10%)"
  )
  ...
  if pack_stage > 1:
   text += (
    ...
    "\n🃏 <b>x10 Расширенных Наборов Карт</b> — <s>250</s> 225 XP (-10%)"
   )
...
async def processing_ten_fpacks_purchase(callback: CallbackQuery):
 await market_purchase(callback, 135, 10, "fpacks", False)
...
async def processing_ten_spacks_purchase(callback: CallbackQuery):
 ...
 stg = row[0] if row else 0
 if stg >= 2:
  await market_purchase(callback, 225, 10, "spacks", False)
...
async def handle_season_leaderboard(callback: CallbackQuery):
 ...
 if season in (0, 1, 2):
  results += (
   ...
   "Топ-3 — <b>x3 текущих наборов карт</b></blockquote>\n\n"
  )
 elif season > 2:
  results += (
   "Награды сезона:\n"
   "<blockquote>Топ-1 — <b>x10 текущих наборов карт</b>\n"
   "Топ-2 — <b>x5 текущих наборов карт</b>\n"
   "Топ-3 — <b>x3 текущих наборов карт</b></blockquote>\n\n"
  )
 rank = 1
 for user in users:
  ...
  point = "" if rank == 1 else "."
  link_start, link_end = url_tg_profile_request(user[0])
  results += f"{link_start}{medal}{point} {escape(user[0])}{link_end} — {int(user[1])} XP\n"
  rank += 1
 cursor.execute("SELECT SUM(total_cards), SUM(xp) FROM users WHERE user_id NOT IN (?, ?)", (dev_id, dev_mini_id))
 row = cursor.fetchone()
 total_cards, total_xp = row if row else (0, 0)
 results += f"\nВсего карточек: {total_cards} | Всего XP: {total_xp}\n\n"
...
async def handle_records_leaderboard(callback: CallbackQuery):
 ...
 if winners:
  ...
  results += "</blockquote>"
 cursor.execute("SELECT full_name FROM users WHERE unlocked_cards = 11 AND user_id NOT IN (?, ?)", (dev_id, dev_mini_id))
 row = cursor.fetchone()
 first_11 = row[0] if row else ""
 if first_11:
  achievers = ['Victony Universal', 'Mᴇ Cʀᴀꜰᴛ♡', 'cwendyzz', 'Mᴇ Cʀᴀꜰᴛ♡', "𔓕 ࣪⊹ ◜⛩️◞ ︎ִ ꩜ すばらしい ꩜ ◜🪽◞ ︎ִ`⌁ ࣪", "Samedy #998", first_11]
  achievements = ['первый в мире <b>Открытый Обычный Набор</b> (23.04)', 'первый в мире <b>Открытый Расширенный Набор</b> (21.06)', 'первая в мире <b>полная коллекция 4/4</b> (26.04)', 'первая в мире <b>полная коллекция 5/5</b> (08.06)', 'первая в мире <b>полная коллекция 9/9</b> (21.06)', 'первая в мире <b>полная коллекция 10/10</b> (28.06)', 'первая в мире <b>полная коллекция 11/11</b>']
 else:
  achievers = ['Victony Universal', 'Mᴇ Cʀᴀꜰᴛ♡', 'cwendyzz', 'Mᴇ Cʀᴀꜰᴛ♡', "𔓕 ࣪⊹ ◜⛩️◞ ︎ִ ꩜ すばらしい ꩜ ◜🪽◞ ︎ִ`⌁ ࣪", "Samedy #998"]
  achievements = ['первый в мире <b>Открытый Обычный Набор</b> (23.04)', 'первый в мире <b>Открытый Расширенный Набор</b> (21.06)', 'первая в мире <b>полная коллекция 4/4</b> (26.04)', 'первая в мире <b>полная коллекция 5/5</b> (08.06)', 'первая в мире <b>полная коллекция 9/9</b> (21.06)', 'первая в мире <b>полная коллекция 10/10</b> (28.06)']
...
async def showing_welcome_message(message: types.Message):
 ...
 "Чтобы начать игру, используй команду /card\n\n"
 "Актуальная версия: v0.1.4.2"
 )
...
async def main():
 ...
 xp = row[0] if row else 0
 if xp == 1217:
  await compensation_for_issued_player()
