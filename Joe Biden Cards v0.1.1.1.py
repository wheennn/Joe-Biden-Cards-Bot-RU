...
CREATE TABLE IF NOT EXISTS users (
 ...
 full_name TEXT,
 first_event_reward INTEGER DEFAULT 0,
 second_event_reward INTEGER DEFAULT 0,
 third_event_reward INTEGER DEFAULT 0,
...
async def reminder():
 while True:
  ...
  for user_id, last_card_time in rows:
   ...
   if last_card_time == 0: continue
   if user_id in waiting_users:
    continue
   else:
    if 86400 <= diff < 93600: # ДВА ЧАСА
     ...
     if random_reminder == 1:
      ...
      await bot.send_message(chat_id=user_id, text=text)
    elif 28800 <= diff < 34200: # ПОЛТОРА ЧАСА
     random_reminder = random.randint(1, 4)
     if random_reminder == 1:
...
async def start_handler(message: types.Message):
 ...
 "UPD: Нажмите /claim чтоб получить подарок\n\n"
 "Актуальная версия: v0.1.1.1 [27.04.2026]"
 )
 await message.answer(text)
